...
export const getRegion = cache(async (): Promise<RequestRegion> => {
  ...
  const address = user
    ? await one<{ country: string }>('select country from addresses where user_id = $1 order by is_default desc, created_at desc limit 1', [user.id])
    : undefined
  ...
  return { currency, country, countryName: COUNTRIES[country].name, rate: rateFor(currency), guestCountryChoice }
})
