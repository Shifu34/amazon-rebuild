...
export default async function CartPage() {
  ...
  return (
    <CartFeedback>
      <div>
        <div className={`mx-auto grid max-w-[1120px] gap-8 px-4 py-10 lg:gap-12 ${active.length || viewed.length ? 'lg:grid-cols-[minmax(0,1fr)_320px]' : ''}`}>
          {/* first in the page so phones, keyboards and screen readers reach the subtotal first; the rail on desktop */}
          {active.length > 0 && (
            <aside aria-label="Cart subtotal" className="card self-start p-6 lg:sticky lg:top-6 lg:col-start-2 lg:row-start-1">
              {itemCount > 0 && !toFree && (
                <p className="mb-4 text-sm text-muted">
                  Ships to {COUNTRIES[country].name} · international shipping is calculated at checkout
                </p>
              )}
              ...
              {itemCount > 0 && toFree && toFree.usdCents > 0 && (
                <div className="mb-4 text-sm text-muted">
                  <progress
                    ...
                    className="mb-2 block h-1.5 w-full appearance-none overflow-hidden rounded-full [&::-moz-progress-bar]:bg-accent [&::-webkit-progress-bar]:bg-line [&::-webkit-progress-value]:bg-accent"
                  />
                  Add <b className="whitespace-nowrap text-danger">{toFree.text}</b> of eligible items to your order to qualify for FREE Shipping.
                </div>
              )}
              <p className="font-display text-xl leading-7">{subtotal}</p>
              {itemCount > 0 ? (
                <Link href="/checkout" className="btn btn-cart btn-lg mt-4 w-full">Proceed to checkout</Link>
              ) : (
                <>
                  <button type="button" disabled className="btn btn-cart btn-lg mt-4 w-full">Proceed to checkout</button>
          ...
          <div className="min-w-0 space-y-4 lg:col-start-1 lg:row-start-1 lg:space-y-5">
            <section>
              {unavailable.length > 0 && (
                <div className="mb-6 rounded-lg border border-deal/40 bg-deal/5 px-4 py-3">
                  <h2 className="text-base">Some items need your attention</h2>
              ...
              {active.length > 0 ? (
                <>
                  <div className="flex items-baseline justify-between gap-4 border-b border-line pb-4">
                    <h1 className="text-[28px] leading-9">Your bag</h1>
                  ...
                  <p className="border-t border-line pt-5 text-right font-display text-xl leading-7">{subtotal}</p>
                </>
              ) : (
                <div className="flex flex-col items-center gap-6 py-4 text-center sm:flex-row sm:items-center sm:text-left">
                  <BagIcon className="size-24 shrink-0 text-line" />
                  <div>
                    <h1 className="text-[26px] leading-9">Your bag is empty</h1>
                    {user ? (
            ...
            {saved.length > 0 && (
              <section aria-labelledby="saved-heading" className="border-t border-line pt-10">
                <h2 id="saved-heading" className="text-xl">Saved for later ({plural(saved.length, 'item')})</h2>
                <ul className="grid grid-cols-2 gap-x-6 gap-y-8 pt-6 sm:grid-cols-3 xl:grid-cols-5">
                  {saved.map((l) => (
            ...
            <p className="text-xs text-muted">
              Prices and availability can change. Your bag holds items at their most recent price.
            </p>
          </div>

          {viewed.length > 0 && (
            <aside aria-labelledby="viewed-heading" className="card self-start p-5">
              <h2 id="viewed-heading" className="text-lg leading-6">Recently viewed</h2>
                ...
                {viewed.map((p) => (
                  <li key={p.id} className="flex gap-3">
                    ...
                    <div className="min-w-0 text-sm">
                      ...
                      <p className="price font-medium">{formatDollars(p.price, currency, rate)}</p>
