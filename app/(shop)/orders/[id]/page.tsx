...
export default async function OrderDetailsPage({ params, searchParams }: Props) {
  ...
  return (
    <div className="mx-auto max-w-[1120px] px-4 py-10">
      ...
      <h1 className="text-[28px] leading-9">Order Details</h1>
      <div className="mt-2 flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 text-sm text-muted">
        <p>
          ...
          <span className="price block whitespace-nowrap sm:inline">Order # {order.id}</span>
        </p>
        <Link href={`/orders/${order.id}/invoice`} className="link">View invoice</Link>
      </div>
      ...
      <div className="mt-8 flex flex-col gap-8">
        {cancelled && view.cancelledCents > 0 && (
          <div role="status" className="flex gap-3 rounded-lg border border-line bg-accent-soft px-4 py-3 text-sm">
            <CheckCircleIcon className="size-5 shrink-0 text-accent" />
            {view.status === 'cancelled' ? (
              <p className="font-medium text-accent">This order has been cancelled.</p>
            ) : (
              <p>
                <b className="block font-medium text-accent">{plural(Number(cancelled) || 1, 'item')} cancelled.</b>
        ...
        {order.paymentKind === 'cod' && !order.confirmedAt && !order.cancelledAt && (
          <section aria-label="Confirm this order" className="rounded-[10px] border border-accent/35 bg-accent-soft p-5 max-md:order-1">
            <h2 className="text-lg">Awaiting your confirmation</h2>
            <p className="mt-1 text-sm text-muted">We won&apos;t ship this cash order until you confirm you want it.</p>
        ...
        {order.refusedAt && (
          <section aria-label="Refused" className="card p-5 text-sm max-md:order-1">
            <h2 className="text-lg">Parcel refused</h2>
            <p className="mt-1 text-muted">Nothing was charged. Cash orders now ask for part of the total up front until your next delivery is taken.</p>
          </section>
        )}

        <section aria-label="Order summary" className="grid gap-6 border-y border-line py-6 text-sm max-md:order-1 md:grid-cols-3 md:gap-10">
          <div>
            <h2 className="mb-1 text-base">Ship to</h2>
            <AddressLines shipTo={order.shipTo} />
          </div>
          <div>
            <h2 className="mb-1 text-base">Payment method</h2>
          ...
          <div>
            <h2 className="mb-1 text-base">Order Summary</h2>
            <OrderTotals order={order} view={view} />
          </div>
        </section>

        <section aria-labelledby="shipment-status" className="card p-5 md:p-6">
          <div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_220px] md:gap-8">
            <div>
              <h2 id="shipment-status" className="text-xl leading-7">{view.headline}</h2>
              <p className="mt-0.5 text-sm text-muted">{view.subline}</p>
              {view.pooledNote && <p className="mt-1 text-sm text-muted">{view.pooledNote}</p>}
              {view.step >= 0 && (
            ...
            <ul className="space-y-6 border-t border-line pt-6 md:col-start-1 md:row-start-2">
              {view.items.map((i) => {
        ...
        {view.step >= 0 && (!delivered || view.returnPending) && (
          <section aria-label="Demo controls" className="rounded-[10px] border border-dashed border-line p-5">
      ...
      <Link href="/orders" className="link mt-8 inline-block text-sm">‹ Back to Your Orders</Link>
    </div>
  )
}
