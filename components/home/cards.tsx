...
export function QuadCard({ title, href, tiles, tint, eager = false }: Card & { tint: string; eager?: boolean }) {
  return (
    <div className="flex flex-col rounded-lg border border-line bg-white p-3 pt-4">
      <h2 className="font-display text-[22px] leading-6 font-extrabold">
        <Link href={href} className={`group flex items-start justify-between gap-4 ${focus}`}>
          <span className="group-hover:underline">{title}</span>
          <ChevronIcon className="-mt-0.5 -mr-1.5 size-7 shrink-0 [&_path]:[stroke-width:1.8]" />
        </Link>
      </h2>
        ...
        {tiles.map((t) => (
          <li key={t.href + t.label}>
            <Link href={t.href} className={`group block ${focus}`}>
              ...
              <span className="mt-2 line-clamp-2 h-10 text-sm leading-5 group-hover:underline">{t.label}</span>
            </Link>
          </li>
        ))}
      </ul>
